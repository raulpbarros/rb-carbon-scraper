...
#: The window's and the EXE's icon. Not in `SEEDED_FILES` — it is read-only and
#: nobody edits it, so it stays in the bundle rather than being copied into the
#: user's profile. Generated by `python packaging/make_icon.py`.
...
def datas(*, seed: Path | None = None) -> list[tuple[str, str]]:
    ...
    *stale* one would be the worse failure, so `build.ps1` rebuilds it every
    ...
    if ICON.exists():
        pairs.append((str(ICON), str(ICON.parent.relative_to(ROOT))))
    if seed.exists():
