...
# The window's and the EXE's icon. Read-only and deliberately NOT in
# SEEDED_FILES: it is not something a user edits, so it stays in the bundle
# rather than being copied into their profile. Generated by
# `python packaging/make_icon.py`.
APP_ICON = BUNDLED_ASSETS_DIR / "app.ico"
...
SYNC_ESTIMATE_MINUTES = {
    ...
    ACR: 120,
    # Eight requests carry every credit record in the registry — each of the
    # four reports is one GET for the session and the printed total, then one
    # POST that downloads the whole thing as CSV. The other ~1,277 requests are
    # the per-project detail pages, which are the only place a crediting period
    # appears. ~1,285 in total, about 18 MB. Measured 2026-08-05; see
    # docs/api-contract-car.md.
    #
    # Twenty-five minutes at the default ~1 req/s, rounded up: **no rate limit
    # was observed, and 28 requests is not evidence of none.** ACR's Cloudflare
    # rule only bit at ~100 in ten minutes, and this is the same order of
    # volume. If a long run starts taking 429s, this registry gets its own
    # `requests_per_second` the way ACR did — and the estimate moves with it.
    CAR: 25,
}
...
# --- Climate Action Reserve, on Xpansiv/APX --------------------------------
# The **fifth platform** here, after S&P Platts, legacy Markit, EcoRegistry and
# ICE GreenTrace. One classic-ASP report module (`myrpt.asp`, the `xxxx2` form,
# `rptdownload.asp`) serves every APX tenant, so the platform code lives in
# `registries/apx/` and `registries/car/` is identity only — the same shape as
# platts/verra, markit/planvivo and greentrace/acr.
#
# **Climate Forward** (`climateforward.apx.com`, 36 projects) is the other
# offset tenant and was checked, not assumed: identical forms, identical field
# names, identical `submitform2`/`downloadnow`. It is one subclass away and has
# no adapter, because its units are *ex-ante forecast* credits rather than
# issued offsets — a different unit and a business decision, not a scraping
# one. The remaining `*.apx.com` tenants (NAR, TIGR, I-REC, MiQ, Digital Fuels)
# are renewable-energy and fuel certificates and are not tCO2e at all.
#
# **ACR used to be here and is not any more.** `acr2.apx.com` still answers,
# with HTTP 200 and "You have reached an invalid page" for every path. See the
# ACR section above. Measured 2026-08-05; see docs/api-contract-car.md.

CAR_SITE = "https://thereserve2.apx.com"

# The public "Report Center" reports this tenant publishes, by their `r=` id.
# Scraped from the site's own Public Reports menu; four of the twelve are
# ingested. `211 Compliance Projects` is an ARB/WA-ECO view of projects already
# in 111, and `706 Buffer Pool Account Balance` is reserve balances — neither
# is a new row set.
CAR_REPORTS = {
    "projects": 111,
    "issuances": 112,
...
# The per-project page, and the **only** source of a crediting period: the
# projects report publishes none of the three dates. Keyed on the numeric part
# of the published reference (`CAR1957` -> `id1=1957`), which was checked on
# every grid row available — 227 rows, 216 distinct projects, 0 mismatches — so
# the URL is derivable without the grid's hyperlink, which the CSV strips
# anyway.
#
# **A bad id is a soft 404**: `id1=999999` returns HTTP 200 and a 3 KB page
# reading "Invalid URL, the Reserve Administrator has been notified!".
# `raise_for_status()` sees nothing, exactly like EcoRegistry's `ERROR_401`.
CAR_PROJECT_DETAIL_URL = f"{CAR_SITE}/mymodule/reg/prjView.asp?id1={{project_id}}"

# Asserted by the adapter. What the registry publishes per project is a
# **protocol** ("Forestry - MX - Version 2.0", "Adipic Acid - Version 1.0") and
# never a standard name; the protocol is the methodology and goes to that
# column instead.
CAR_STANDARD_NAME = "Climate Action Reserve"
